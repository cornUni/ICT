...
class CLI():
    ...
    def execute(self):
        ...
        if self._args.command == str(Actions.CREATE):
            self._action.create(self._args.passwordName, " ".join(self._args.description), self._args.key)
        ...
        elif self._args.command == str(Actions.MASS):
            self._action.mass(self._args.key)
